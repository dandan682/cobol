...
# define DBPREVERSIONLONG "V 0.4 2020-08-22"

char infilename[256];
int tabstop=3;
int freeformat=FALSE;
			...
			if(f==0){
				if(strcmp(option, "-h")==0 ||
				   strcmp(option, "--help")==0){
					...
					puts("-f / --free    - free format source code formatting allowed");
					puts("");
					puts("Developed by the_piper@web.de");
					puts("For more infos see here: https://sourceforge.net/p/dbpre/discussion/?source=navbar");
					exit(0);
				}
			}
			if(f==0 && strcmp(option, "-f")==0){
				freeformat=TRUE;
				printf("setting option freeformat=TRUE\n");
				f=1;
			}
			if(f==0 && strcmp(option, "--free")==0){
				freeformat=TRUE;
				printf("setting option freeformat=TRUE\n");
				f=1;
			}
...
void parse_fetch_statement(FILE *fp, FILE *fplst)
{
	...
	if(freeformat==TRUE){
		sprintf(zwi, "      IF SQLCA-CURSOR-CTRL (%d) = 0", idx+1);
		do_output(zwi);
		do_output("         SET DB-CURSOR-NOT-OPEN TO TRUE");
		do_output("         PERFORM DB-STATUS");
		do_output("      END-IF");
		sprintf(zwi,"         CALL 'MySQL_fetch_row' USING SQLCA-RESULT (%d)", idx+1);
		do_output(zwi);
		for(t=1;;t++){
		...
		do_output("         END-CALL");
		sprintf(zwi, "         IF SQLCA-RESULT (%d) = NULL", idx+1);
		do_output(zwi);
		do_output("            MOVE 100 TO SQLCODE");
		do_output("         ELSE");
		do_output("            MOVE 0 TO SQLCODE");
		do_output("         END-IF");
	}
	else{
...
void parse_delete_statement(FILE *fp, FILE *fplst)
{
	...
	stage=0;
	if(freeformat==TRUE){
		do_output("  MOVE LOW-VALUES TO SQLCA-STATEMENT");
		do_output("  STRING");
	}
	else{
	...
	for(t=1;;t++){
		...
		if(zwi[0]==':'){
			if(freeformat==TRUE){
				do_output("  SQLCA-QUOTE DELIMITED SIZE");
				sprintf(zwi2, "  %s DELIMITED SIZE", zwi+1);
				do_output(zwi2);
				do_output("  SQLCA-QUOTE DELIMITED SIZE");
				do_output("  ' ' DELIMITED SIZE");
			}
			else{
			...
			strcat(zwi, " ");
			if(freeformat==TRUE){
				sprintf(zwi2, "  '%s' DELIMITED SIZE", zwi);
			}
			else{
	...
	if(freeformat==TRUE){
		do_output("  INTO SQLCA-STATEMENT");
		do_output("  END-STRING");
		do_output("  CALL 'MySQL_query' USING SQLCA-STATEMENT");
		do_output("  END-CALL");
		do_output("  MOVE RETURN-CODE TO SQLCODE");
	}
	else{
	...
	for(t=1;;t++){
		...
		if(zwi[0]==':'){
			...
			if(zwi[0]=='\''){
				sprintf(zwi2, "'%s' ", zwi);
				if(freeformat==TRUE){
					sprintf(zwi, "  '%s' DELIMITED SIZE", zwi2);
				}
				else{
...
void parse_cursor_statement(FILE *fp, FILE *fplst, int idx)
{
	...
	if(freeformat==TRUE){
		...
		do_output(zwi);
		sprintf(zwi, "      IF SQLCA-CURSOR-CTRL (%d) = 1", idx+1);
		do_output(zwi);
		do_output("         SET DB-CURSOR-ALREADY-OPEN TO TRUE");
		do_output("         PERFORM DB-STATUS");
		do_output("      END-IF\n");
		sprintf(zwi, "      MOVE 1 TO SQLCA-CURSOR-CTRL (%d)", idx+1);
		do_output(zwi);
		do_output("      MOVE LOW-VALUES TO SQLCA-STATEMENT");
		for(t=0;;t+=25){
			...
			zwi[25]=0;
			sprintf(zwi2, "      MOVE '%s' TO SQLCA-STAT-LINE (%d)", zwi, z);
			do_output(zwi2);
			z++;
		}
		do_output("      CALL 'MySQL_query' USING SQLCA-STATEMENT");
		...
		do_output("      MOVE RETURN-CODE TO SQLCODE");
		do_output("      IF DB-OK");
		sprintf(zwi, "         CALL 'MySQL_use_result' USING SQLCA-RESULT (%d)", idx+1);
		do_output(zwi);
		do_output("         END-CALL");
		sprintf(zwi, "         IF SQLCA-RESULT (%d) = NULL", idx+1);
		do_output(zwi);
		...
		//fprintf(fp, "         CALL 'MySQL_fetch_fields' USING SQLCA-RESULT (%d)\n", idx+1);
		sprintf(zwi, "         CALL 'MySQL_fetch_row' USING SQLCA-RESULT (%d)", idx+1);
		do_output(zwi);
		for(t=1;;t++){
			word(variables, zwi, t);
                                                ...
                                                unsigned char *fullline)
{
	...
	if(freeformat==TRUE){ // comment is *>
		for(t=0;;t++){
			if(fullline[t]==0) break;
			if(fullline[t]=='*' && fullline[t+1]=='>'){
				if(t==0){
					return(FALSE);
				}
				else{
	...
	if(strcmp(w1, "EXEC")==0 &&
	   strcmp(w2, "SQL")==0){
		...
		if(proceduremode==TRUE){
			if(freeformat==TRUE){
				sprintf(zwi, "%sMOVE %d             TO SQLCA-SEQUENCE", 
	   ...
	   if(exec_sql_error==TRUE){
		...
		if(strcmp(w1, "INCLUDE")==0){
         ...
         if(fpin[fpincnt]==NULL){
				...
				fprintf(fp, "%s\n", fullline);
				fprintf(fplst, "%s\n", fullline);
				fprintf(fplst, "DBPRE-ERROR: could not include file >%s<.\n", w2);
				return(TRUE);
			}
			if(freeformat==TRUE){
				sprintf(zwi, "*> DBPRE %s", fullline);
		...
		if(strcmp(w1, "INIT")==0 && strcmp(w2, "DB")==0){
			...
			do_output(zwi);
			if(freeformat==TRUE){
		 	   sprintf(zwi,"%s CALL \"MySQL_init\"  USING SQLCA-CID",zwioffset);
			   do_output(zwi);
		 	   sprintf(zwi, "%s END-CALL", zwioffset);
			   do_output(zwi);
		 	   sprintf(zwi, "%s MOVE RETURN-CODE    TO SQLCODE", zwioffset);
			   do_output(zwi);
			}
			else{
		...
		if(strcmp(w1, "CLOSE")==0 && strcmp(w2, "DB")==0){
			...
			do_output(zwi);
			if(freeformat==TRUE){
		 		sprintf(zwi,"%s CALL \"MySQL_close\"\n",zwioffset);
				do_output(zwi);
				sprintf(zwi, "%s END-CALL\n", zwioffset);
				do_output(zwi);
				sprintf(zwi, "%s MOVE RETURN-CODE    TO SQLCODE\n",zwioffset);
				do_output(zwi);
			}
			else{
		...
		if(strcmp(w1, "CLOSE")==0 && strcmp(w2, "DB")!=0){
			...
			if(freeformat==TRUE){
				sprintf(zwi, "*> DBPRE %s", fullline);
				do_output(zwi);
				sprintf(zwi, "      IF SQLCA-CURSOR-CTRL (%d) = 0", t+1);
				do_output(zwi);
				do_output("         SET DB-CURSOR-NOT-OPEN TO TRUE");
				do_output("         PERFORM DB-STATUS");
				do_output("      END-IF");
				sprintf(zwi, "      MOVE 0 TO SQLCA-CURSOR-CTRL (%d)", t+1);
				do_output(zwi);
			}
			else{
				strcpy(zwi, fullline);
		...
		if(strcmp(w1, "CONNECT")==0 && strcmp(w2, "DB")==0){
			...
			if(freeformat==TRUE){
				sprintf(zwi, "*> DBPRE %s", fullline);
				do_output(zwi);
	 			sprintf(zwi, "%s CALL \"MySQL_real_connect\" USING",zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s                           SQLCA-HOST", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s                           SQLCA-USER", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s                           SQLCA-PASSWD", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s                           SQLCA-DBNAME", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s                           SQLCA-PORT", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s                           SQLCA-SOCKET", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s END-CALL", zwioffset);
				do_output(zwi);
	 			sprintf(zwi, "%s MOVE RETURN-CODE    TO SQLCODE", zwioffset);
		...
		if(strcmp(w1, "SELECT")==0 && strcmp(w2, "DB")==0){
			...
			if(freeformat==TRUE){
				sprintf(zwi, "*> DBPRE %s", fullline);
				do_output(zwi);
		 		sprintf(zwi,"%s CALL \"MySQL_selectdb\" USING SQLCA-DBNAME",zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s END-CALL", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s MOVE RETURN-CODE    TO SQLCODE", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s IF RETURN-CODE NOT = 0 THEN", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s    PERFORM DB-STATUSR", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s END-IF", zwioffset);
				do_output(zwi);
			}
			else{
				strcpy(zwi, fullline);
		...
		if(strcmp(w1, "COMMIT")==0){
			...
			if(freeformat==TRUE){
				sprintf(zwi, "*> DBPRE %s", fullline);
				do_output(zwi);
		 		sprintf(zwi,"%s CALL \"MySQL_commit\"", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s END-CALL\n", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s MOVE RETURN-CODE    TO SQLCODE", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s IF RETURN-CODE NOT = 0 THEN", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s    PERFORM DB-STATUS", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s END-IF", zwioffset);
				do_output(zwi);
			}
		...
		if(strcmp(w1, "ROLLBACK")==0){
			...
			if(freeformat==TRUE){
				sprintf(zwi, "*> DBPRE %s", fullline);
				do_output(zwi);
		 		sprintf(zwi,"%s CALL \"MySQL_rollback\"", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s END-CALL", zwioffset);
				do_output(zwi);
		 		sprintf(zwi, "%s MOVE RETURN-CODE    TO SQLCODE", zwioffset);
